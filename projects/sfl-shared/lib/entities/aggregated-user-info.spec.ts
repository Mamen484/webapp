...
describe('AggregatedUserInfo', () => {
    ...
    it('findFirstDemoStore should return the first store with a status "demo"', () => {
        let userInfo = AggregatedUserInfo.create({_embedded: {store: [
                    {status: 'deleted', id: 3},
                    {status: 'demo', id: 4},
                ]}});
        expect(userInfo.findFirstDemoStore().id).toEqual(4);

        userInfo = AggregatedUserInfo.create({_embedded: {store: [
                    {status: 'deleted', id: 5},
                    {status: 'suspended', id: 6},
                ]}});
        expect(userInfo.findFirstDemoStore()).toEqual(undefined);
    });
