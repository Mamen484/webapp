...
import { data, data2 } from '../../../../src/mocks/updates-for-timeline-service.mock';
...
describe('TimelineService', () => {
    ...
    beforeEach(async(() => {
        ...
        TestBed.configureTestingModule({
            providers: [
                ...
                {provide: SFL_API, useValue: 'http://apiLink.com/v1'},
            ],
    ...
    it('should create an array of distinct updates: only the one last import and one last export of each channel', () => {
        ...
        let req = httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline?name=feed.export,feed.import` +
    ...
    it('should pass proper params on getEvents call', () => {
        ...
        httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline?limit=50&name=feed.import,feed.export,order.lifecycle,` +
    ...
    it('should fetch appropriate link on getEventsByLink call', () => {
        ...
        httpMock.expectOne(`http://apilink.com/v1/store/909/timeline?name=feed.import&feed.export&order.lifecycle&` +
    ...
    it('should pass proper params on getEventUpdates call', () => {
        ...
        httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline?name=feed.export,feed.import&since=2017-06-13T02:22:41.000Z&` +
    ...
    it('should use TimelineFilter to set params of getEvents request', () => {
        ...
        httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline?limit=50&name=rule.segmentation,order.lifecycle&action=delete,ship&` +
    ...
    it('should fetch both events and updates when timeline update emitted', () => {
        ...
        httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline?name=feed.export,feed.import&` +
            `since=2011-12-10T00:00:00.000Z&limit=200&action=ask,start,finish,error,cancel`);
        httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline?limit=50&name=feed.import,feed.export,order.lifecycle,` +
            `rule.transformation,rule.segmentation&action=create,push,delete,ship,update,error,cancel`);
        allowNoExpectations();
    ...
    it('should pass to subscribers two events: fetch start (empty) and fetch finish (with data) when updated timeline is requested', (done) => {
        ...
        const updates = httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline?name=feed.export,feed.import&` +
            `since=2011-12-10T00:00:00.000Z&limit=200&action=ask,start,finish,error,cancel`);
        const events = httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline?limit=50&name=feed.import,feed.export,order.lifecycle,` +
            `rule.transformation,rule.segmentation&action=create,push,delete,ship,update,error,cancel`);
    ...
    it('should send HEAD request to the timeline resource when updates number requested', () => {
        ...
        let req = httpMock.expectOne(`http://apiLink.com/v1/store/909/timeline`);
